...
print(f"Full embeddings generated in {timeForFull:.2f} seconds.\n")
print(f"Memory used for full embeddings: {memoryForFull:.2f} MB\n")
print(f"Full embeddings shape is {fullEmbeddings.shape}\n")
...
print("Compressing embeddings from 384 dimensions to 64...")
startTime = time.time()
...
print(f"Compressed embeddings (64 dimensions) generated in {timeForPCA64:.2f} seconds.\n")
print(f"Memory used for compressed embeddings (64 dimensions): {memoryForPCA64:.2f} MB\n")
print(f"Compressed embeddings shape: {compressedEmbeddings64.shape}\n")
print("Expected shape: (500, 64)\nGot shape:", compressedEmbeddings64.shape, "\n")

#384-dimensional embeddings are reduced to 32 dimensions
print("Compressing embeddings from 384 dimensions to 32...")
startTime = time.time()
...
print(f"Compressed embeddings (32 dimensions) generated in {timeForPCA32:.2f} seconds.\n")
print(f"Memory used for compressed embeddings (32 dimensions): {memoryForPCA32:.2f} MB\n")
print(f"Compressed embeddings shape: {compressedEmbeddings32.shape}\n")
...
#main
query = "Students looking for affordable housing options near campus"

print("Top results using FULL embeddings:")
for i in search(query, fullEmbeddings):
    print("-", documents[i][:150].replace("\n", " "))

print("\nTop results using COMPRESSED embeddings(64 dimensions):")
for i in search(query, compressedEmbeddings64, pca=pca64):
    print("-", documents[i][:150].replace("\n", " "))

print("\nTop results using COMPRESSED embeddings(32 dimensions):")
...
print("\nEfficiency analysis:")
print(f"-Full 384 dimensions - Time: {timeForFull:.2f} seconds, Memory: {memoryForFull:.2f}MB")
print(f"-Compressed 64 dimensions - Time: {timeForPCA64:.2f} seconds, Memory: {memoryForPCA64:.2f}MB, Recall: {recall64*100:.2f}%")
